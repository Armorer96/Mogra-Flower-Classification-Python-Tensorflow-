...
edged = cv2.Canny(opening,200,200)
...
for c in cnts:
	...
	if dA >=100:
		print("grade I mogra")
		cv2.putText(orig, "Grade I",
		(int(tltrX - 15), int(tltrY-30)), cv2.FONT_HERSHEY_SIMPLEX,
		0.65, (255, 0, 0), 2)
		j=j+1
	else:
		print("grade II mogra")
		cv2.putText(orig, "Grade II",
		(int(tltrX - 15), int(tltrY-30)), cv2.FONT_HERSHEY_SIMPLEX,
		0.65, (0, 255, 0), 2)
		k=k+1
...
outfile.close()
print("Total number of Mogra buds=",i)
print("Grade I mogra=",j)
print("Grade II mogra=",k)
...
graph_data=open('res.txt','r').read()
